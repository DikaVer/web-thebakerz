...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        if (!email || !storeId || !cosmosId || !cartId) {
            const missingParam = !email ? t("missingUser") :
                              !storeId ? t("missingStoreId") :
                              !cosmosId ? t("missingCosmosId") : t("missingCartId");
            await connectionPool.query('ROLLBACK');
        ...
        if (!userSession) {
            await connectionPool.query('ROLLBACK');
            return NextResponse.redirect(new URL(`/${storeIdParam}/order/failed?error=${t("missingUser")}&session_id=${sessionId}`, origin), { status: 308 });
        }
        ...
        if (!dateParams || !timeParams || !cartItems || cartItems.length === 0) {
            const errorType = !dateParams || !timeParams ? t("missingOrderTime") : t("missingCart");
            await connectionPool.query('ROLLBACK');
        ...
        if (result.rows.length === 0) {
            await connectionPool.query('ROLLBACK');
            return NextResponse.redirect(new URL(`/${storeIdParam}/order/failed?error=${t("orderFailed")}&session_id=${sessionId}`, origin), { status: 308 });
        }
        ...
        return NextResponse.redirect(new URL(`/${storeIdParam}/order/failed?error=${t("processingError")}&session_id=${sessionId}`, origin), { status: 308 });
    }
}