...
export async function updateDeliveryAddress(
  ...
  try {
    ...
    const sanitizedAddress = {
      ...address,
      formattedAddress: truncateField(address.formattedAddress, MAX_CHARS.formattedAddress),
      street: truncateField(address.street, MAX_CHARS.street),
      houseNumber: truncateField(address.houseNumber, MAX_CHARS.houseNumber),
      city: truncateField(address.city, MAX_CHARS.city),
      zipCode: truncateField(address.zipCode, MAX_CHARS.zipCode),
      additionalInfo: truncateField(address.additionalInfo, MAX_CHARS.additionalInfo)
    };
    ...
    const querySpec = {
      query: "SELECT * FROM c WHERE c.storeId = @storeId AND c.userId = @userId",
      parameters: [
    ...
    if (existingAddresses && existingAddresses.length > 0) {
      ...
      await containerDeliveryLocations.item(existingAddress.id, partitionKeyValue).replace({
        ...existingAddress,
  ...
  try {
    ...
    if (userAddresses && userAddresses.length > 0) {
      ...
      const guestQuerySpec = {
        query: "SELECT * FROM c WHERE c.storeId = @storeId AND c.userId = @guestId",
        parameters: [
          { name: "@storeId", value: storeId },
