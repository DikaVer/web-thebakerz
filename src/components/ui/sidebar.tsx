      ...
      <SidebarContext.Provider value={contextValue}>
        <TooltipProvider delayDuration={0}>
          <div
            ...
            className={cn(
              "group/sidebar-wrapper min-h-screen w-full has-[[data-variant=inset]]:bg-sidebar",
              className
            )}
            ref={ref}
            {...props}
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      className={cn("h-10 w-9 px-0 py-0 items-center rounded-full transition duration-500 hover:bg-gray-200", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
      {...props}
    >
        <IconMenu className="w-8 h-7 text-text" />
