...
const EditBakerzForm: React.FC<EditBakerzFormProps> = ({ store, businessData, className }) => {
    ...
    const form = useForm<z.infer<typeof OnboardSchema>>({
        ...
        defaultValues: {
            ...
            bankAccount: businessData?.bank_account || "",
            businessRoute: businessData?.location?.route || "",
            businessCity: businessData?.location?.city || "",
            businessZipCode: businessData?.location?.zip_code || "",
            businessCountry: businessData?.location?.country || "",
            regionBusiness: businessData?.region.toUpperCase() || "NL",
            kor: businessData?.kor || false,
        },
    });

    // Use action state for the form submission
    const [state, submitAction, isPending] = useActionState(
        async (previousState: any, formData: z.infer<typeof OnboardSchema>) => {
            if (!store) {
                showErrorMessage({ error: "Store data not found" });
                return;
            }

            const result = await updateBakerz(formData, store.user_id, store.id);
    ...
    return (
        <div className={cn(className)}>
            {/* Profile Section */}
            <div>
                <p className="text-base font-medium text-default-700">Edit Bakerz Profile</p>
                ...
                <Card shadow="none" className="mt-4 bg-default-100">
                    <CardBody>
                        <div className="flex items-center gap-4">
                            <Avatar
                                key={store?.picture} // changing key forces re-mount
                                src={store?.picture}
                            ...
                            />
                            <div>
                                <p className="text-sm font-medium text-default-500">{store?.storeName}</p>
                                <p className="text-xs text-default-400">Bakerz</p>
                                <p className="mt-1 text-xs text-default-400">{store?.email}</p>
                            </div>
                        </div>
                    </CardBody>
                </Card>
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(handleSubmit)} className="grid gap-y-4">
                    ...
                    <div>
                        ...
                        <p className="mt-1 text-sm font-normal text-default-400">Edit contact email</p>
                        <FormField
                            control={form.control}
                            name="email"
                            render={({ field, fieldState }) => (
                    ...
                    <div>
                        ...
                        <p className="mt-1 text-sm font-normal text-default-400">Bakerz Stripe Account</p>
                        <FormField
                            control={form.control}
                    ...
                    <div className="flex flex-row-reverse w-full">
                        <Button
                            ...
                            isLoading={isPending}
                        >
                            {isPending ? "Updating..." : "Update Bakerz"}
                        </Button>
                    </div>
                </form>
            </Form>
